...
template<class Context>
class my_basic_format_arg<Context>::handle {
	using charT = typename Context::char_type;
	using format_function = void(*)(std::basic_format_parse_context<charT>&, Context&, const void*);
	...
	template<class T> explicit handle(const T& val) noexcept
	  :ptr_(addressof(val))
	  ,format_(
		  [](std::basic_format_parse_context<charT>& parse_ctx, Context& format_ctx, const void* ptr) {
	...
	friend class my_basic_format_arg<Context>;

public:
	void format(std::basic_format_parse_context<charT>& parse_ctx, Context& format_ctx) const
	{
	...
	while (!std::ranges::empty(pctx))
	{
		...
		if (*it == '{')
		{
			...
			my_visit_format_arg([&]<class T>(const T& value)
			{
				...
				else if constexpr (std::same_as<T, typename my_basic_format_arg<Context>::handle>)
				{
					value.format(pctx, fctx);
				}
				else
				{
...
int main()
{
	...
	std::cout << my_format("{}", std::chrono::system_clock::now()) << '\n';
}
