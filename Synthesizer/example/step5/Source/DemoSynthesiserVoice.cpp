...
void SynthesiserVoice::renderNextBlock(juce::AudioBuffer<float> &outputBuffer, int startSample, int numSamples)
{
    ...
    if (isKeyDown() || isPlayingButReleased())
    {
        ...
        for (int sample = 0; sample < numSamples; sample++)
        {
            ...
            {
                double sampleData = processSample(m_currentAngle, waveType) * m_velocity * fade * volume;
...
double SynthesiserVoice::processSample(double angle, WaveType waveType)
{
    ...
    else if (waveType == WaveType::Square || waveType == WaveType::Triangle)
    {
        ...
        {
            value = m_angleDelta * value + (1.0 - m_angleDelta) * m_lastValue;
            m_lastValue = value;
        }
    }
    // サイン波およびランダムノイズ
