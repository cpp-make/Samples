...
juce::AudioProcessorValueTreeState::ParameterLayout createLayout()
{
    ...
    layout.add(std::make_unique<juce::AudioParameterChoice>(juce::ParameterID("type", 1),
                                                            "type",
                                                            SynthesiserSound::GetWaveTypeList(),
                                                            0));

    layout.add(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID("phase", 1),
                                                           "phase",
...
void DemoSynthesiserAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    // ビジュアライザ用音声データに音声を格納する。
    stereoFifo.writeToFifo(buffer);
}
...
void DemoSynthesiserAudioProcessor::setupSynthesiser()
{
    ...
    m_synthesiser.addSound(new SynthesiserSound(*apvts.getRawParameterValue("volume"),
                                                *apvts.getRawParameterValue("type"),
                                                *apvts.getRawParameterValue("phase"))); 
}
